...
export const AdminDashboard: React.FC = () => {
    ...
    const handleTogglePremium = (targetUser: User, e: React.MouseEvent) => { e.stopPropagation(); const isCurrentlyPremium = userService.isPremium(targetUser); const action = isCurrentlyPremium ? 'Remover Premium' : 'Conceder Premium'; setModalConfig({ title: `${action}`, message: isCurrentlyPremium ? `Remover o Cine Safe Premium de ${targetUser.name}? O referralCount será zerado.` : `Conceder Cine Safe Premium para ${targetUser.name}? Isso libera inventário, verificações e contatos ilimitados.`, isDestructive: isCurrentlyPremium, confirmLabel: action, action: async () => { const newCount = isCurrentlyPremium ? 0 : 5; await supabase.from('users').update({ referral_count: newCount }).eq('id', targetUser.id); await loadData(); setModalOpen(false); } }); setModalOpen(true); };
                        ...
                        {sortedUsers.map(u => (
                            <div key={u.id} onClick={() => handleUserClick(u)} className={`glass-card p-6 rounded-2xl border transition-all cursor-pointer group relative overflow-hidden ${u.isBlocked ? 'border-red-500/30 opacity-75 grayscale' : 'border-white/5 hover:border-accent-primary/30'}`}>
                                ...
                                <div className="flex flex-wrap justify-between gap-2">
                                    <button onClick={(e) => handleTogglePremium(u, e)} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-colors border ${userService.isPremium(u) ? 'bg-purple-500/10 text-purple-400 border-purple-500/20 hover:bg-purple-500/20' : 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20 hover:bg-yellow-500/20'}`}>{userService.isPremium(u) ? '⭐ Remover Premium' : '⭐ Dar Premium'}</button>
                ...
                <div className="fixed inset-0 z-[4000] flex items-center justify-center p-4 bg-brand-950/90 backdrop-blur-xl animate-fade-in" onClick={closeUserModal}>
                    <div className="glass-card max-w-lg w-full p-8 rounded-[2.5rem] relative border border-white/10 shadow-2xl" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex flex-wrap gap-3">
                            <button onClick={(e) => handleTogglePremium(selectedUser, e)} className={`flex-1 py-3 font-bold rounded-xl border transition-colors ${userService.isPremium(selectedUser) ? 'bg-purple-500/10 text-purple-400 border-purple-500/20 hover:bg-purple-500/20' : 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20 hover:bg-yellow-500/20'}`}>{userService.isPremium(selectedUser) ? '⭐ Remover Premium' : '⭐ Dar Premium'}</button>
