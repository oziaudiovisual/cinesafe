    ...
    <div className="space-y-6 animate-fade-in pb-10">
        ...
        <div>
            ...
            <div className="bg-brand-800/50 rounded-[2.5rem] border border-white/5 p-6 md:p-8 grid grid-cols-2 md:grid-cols-3 gap-6 md:gap-8 relative overflow-hidden">
                ...
                {/* Mínimos de prova social — conforme os reais crescem, eles assumem */}
                <GlobalStatItem icon={Icons.Camera} label="Equipamentos Protegidos" value={Math.max(318, systemStats.totalItems).toLocaleString('pt-BR')} />
                <GlobalStatItem icon={Icons.ShoppingBag} label="Transações Protegidas" value={Math.max(117, systemStats.transactionsCount || 0).toLocaleString('pt-BR')} />
                <GlobalStatItem icon={Icons.Banknote} label="Valor Movimentado" value={Math.max(412000, systemStats.transactedValue || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', notation: 'compact' })} />
                <GlobalStatItem icon={Icons.Wallet} label="Em Valor Segurado" value={Math.max(113718, systemStats.totalValue).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', notation: 'compact' })} />
                <GlobalStatItem icon={Icons.ShieldCheck} label="Itens Recuperados" value={Math.max(12, systemStats.recoveredItems).toLocaleString('pt-BR')} />
                <GlobalStatItem icon={Icons.Siren} label="Ocorrências Registradas" value={Math.max(18, systemStats.stolenItems).toLocaleString('pt-BR')} />
            </div>
        </div>
    </div>
  );
