...
export const equipmentService = {
  ...
  cancelTransfer: async (equipmentId: string): Promise<boolean> => {
    try {
      const batch = writeBatch(db);
      const notifQuery = query(collection(db, 'notifications'), where('itemId', '==', equipmentId), where('type', '==', 'ITEM_TRANSFER'));
      const notifSnap = await getDocs(notifQuery);
      if (!notifSnap.empty) {
        batch.delete(notifSnap.docs[0].ref);
      }
      const equipmentDocRef = doc(db, 'equipment', equipmentId);
      batch.update(equipmentDocRef, { status: EquipmentStatus.SAFE, pendingTransferTo: null });
